...
public class RequestTimeConsumingInterceptor implements HandlerInterceptor {
    ...
    public void afterCompletion(HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse, Object o, Exception e) {
        ...
        StringBuilder stringBuilder = new StringBuilder(userIdStr);
        for(int i=0,len=userIdStr.length(); i<8-len; i++) {
            stringBuilder.append(" ");
        }
        StringBuilder stringBuilder1 = new StringBuilder(servletPath);
        for(int i=0,len=servletPath.length(); i<60-len; i++) {
            stringBuilder1.append(" ");
        }
        StopWatch stopWatch = stopWatchThreadLocal.get();
        stopWatch.stop();
        logger.info("{}{}{}ms", stringBuilder.toString(), stringBuilder1.toString(), stopWatch.getTotalTimeMillis());
    }
}
